...
H(X) = -\log(S(X))
$$
where $S(X)$ is the survival function. This can be used to evaluate overall model fit, since
under correct specificiation, $H(X)$ follows an exponential distribution with rate 1. So the model fit can be graphically
evaluated by seeing agreement between the cumulative hazard function of the Cox-Snell Residuals against the actual 
residuals, which should be linear. One could also consider plotting the Kaplan-Meier curve for these residuals 
against the exponential distribution with rate 1. 
